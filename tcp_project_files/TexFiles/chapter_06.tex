...
All three curves show cumulative germination over time. Curve a starts early and rises steeply, reflecting high vigour and uniform germination. Curve b begins later and increases more slowly but eventually reaches a similar final level, indicating viable seeds with lower vigour or mild dormancy. Curve c rises slowly and never reaches full germination, suggesting aged or partially dormant seeds with reduced viability.

2) Are they as expected for an ordinary germination test?
Yes. Under controlled ISTA test conditions-stable temperature, light, and moisture-cumulative germination typically follows a sigmoidal pattern. Seeds germinate asynchronously due to individual physiological differences, and variation among lots mainly appears in onset time, rate, and total percentage.

3) How to estimate the germination curve mathematically?
The time-course data can be fitted to the cumulative log-logistic function:
\[ F(t) = \frac {1}
                {1 + \left( \frac{t}{t_{50}} \right)^{b}} \]

where F(t) is the fraction germinated at time t. This model describes the sigmoidal rise and allows quantitative comparison among seed lots.

4) Necessary parameters in the model.
Key parameters include t50- the time required for 50 \% germination, representing the median rate - and the shape parameter b, which defines the steepness and synchrony of germination. The upper asymptote represents the maximum germination percentage.

5) How to test whether seed lots a and b differ significantly?
Fit the log-logistic model separately to each time series and compare estimated parameters (t50, b, maximum \%). Statistical differences can be tested via confidence-interval overlap or ANOVA on model parameters. A shorter t50 indicates faster germination and greater vigour.

6) Why does seed lot c have another shape than a and b?
Curve c shows a lower final germination and a flatter slope, reflecting heterogeneous or damaged seeds. This could arise from ageing, mechanical injury, or partial dormancy, all of which slow and reduce germination even under the same test conditions.

7) If lots a and b are identical but a is from an ordinary Curve b could result from a seedling emergence test conducted in soil rather than a paper germination test. In soil, physical resistance, temperature fluctuation, and oxygen limitation delay and spread germination events, producing a slower, flatter curve even for the same seed lot.
